...
function handleMessageToAll(socket, io, nickNames) {
  socket.on('all', function (message) {
    io.sockets.emit('all', {
      text: nickNames[socket.id] + ': ' + message
    });
  });
}
...
exports.listen = function(server) {
  ...
  io.sockets.on('connection', function (socket) {
    ...
    // Handle user messages, name change attempts, room creation/changes, etcetera.
    handleMessageToAll(socket, io, nickNames);
    handleMessageBroadcasting(socket, nickNames);
    handleNameChangeAttempts(socket, nickNames, namesUsed);
    handleRoomJoining(socket);

    // Provide user with a list of guests in their room on request.
    socket.on('guests', function(room) {
      var clients = io.sockets.clients(room);
      var guestNames = [];

      for (var i = 0; i < clients.length; i++) {
        guestNames.push(nickNames[clients[i]['id']]);
      }

      socket.emit('guests', guestNames);
    });
